...
public class DateUtil {
    ...
    public static String getPreDate(final String dateString) {
        final Date date = stringToDate(dateString, "yyyy-MM-dd", DateUtil.LENIENT_DATE);
        final Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
        cal.setTime(date);
        final Calendar calNew = cal;
        calNew.set(5, calNew.get(5) - 1);
        return dateToString(calNew.getTime(), "yyyy-MM-dd");
    }
    
    public static String getPreWorkDate(final String dateString) {
        final Date date = stringToDate(dateString, "yyyy-MM-dd", DateUtil.LENIENT_DATE);
        final Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
        now.setTime(date);
        final int today = now.getTime().getDay();
        if (today == now.getFirstDayOfWeek()) {
            now.roll(6, -3);
        }
        else if (today == 0) {
            now.roll(6, -2);
    ...
    public static String stringDateFormat(final String dateString, final String format) {
        return dateToString(stringToDate(dateString, format, DateUtil.LENIENT_DATE), format);
    }
    ...
    static {
        ...
        DateUtil.LENIENT_DATE = false;
        DateUtil.random = new Random();
    }
