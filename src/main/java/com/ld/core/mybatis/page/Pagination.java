package com.ld.core.mybatis.page;
...
public class Pagination<T> extends SimplePage implements java.io.Serializable,
	...
	public String getPageHtml(){
	  ...
	  public void setFinacePageHtml(String methodeName)
	  {
	    StringBuffer finacePageHtml = new StringBuffer("<ul class='pagination'>");
	    if (getPageNo() > 1) {
	      if (getPageNo() > 5) {
	        finacePageHtml.append("<li><a href='javascript:;' onclick='" + methodeName + "(1)'>首页</a></li>");
	      }
	      finacePageHtml.append("<li><a href='javascript:;'  onclick='" + methodeName + "(" + (getPageNo() - 1) + ")'>上一页</a></li>");
	    }
	    int i = getPageNo() - 2 <= 0 ? 1 : getPageNo() - 2; for (int no = 1; (i <= getTotalPage()) && (no < 6); no++) {
	      if (getPageNo() == i)
	        finacePageHtml.append("<li class='active'><a href='javascript:void(0);' >" + i + "</a></li>");
	      else
	        finacePageHtml.append("<li><a href='javascript:;' onclick='" + methodeName + "(" + i + ")'>" + i + "</a></li>");
	      i++;
	    }

	    if (getPageNo() < getTotalPage()) {
	      finacePageHtml.append("<li><a href='javascript:;'  onclick='" + methodeName + "(" + (getPageNo() + 1) + ")'>下一页</a></li>");
	    }
	    finacePageHtml.append("</ul>");
	    this.finacePageHtml = finacePageHtml.toString();
	  }

	  public String getFinacePageHtml() {
	    return this.finacePageHtml;
	  }
